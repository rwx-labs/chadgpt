...
describe("removeRedundantNewlines", () => {
  ...
  it("should handle empty string", () => {
    expect(messageUtils.removeRedundantNewlines("")).toBe("");
  });

  it("should handle string with no newlines", () => {
    expect(messageUtils.removeRedundantNewlines("No newlines here")).toBe("No newlines here");
  });

  it("should handle string starting with newlines", () => {
    expect(messageUtils.removeRedundantNewlines("\n\nStart")).toBe("\nStart");
  });

  it("should handle string ending with newlines", () => {
    expect(messageUtils.removeRedundantNewlines("End\n\n")).toBe("End\n");
  });
  ...
  it("should handle mixed newlines and spaces", () => {
    expect(messageUtils.removeRedundantNewlines("Text \n\n Text")).toBe("Text \n Text");
  });

  it("should handle Windows line endings", () => {
    expect(messageUtils.removeRedundantNewlines("Line1\r\n\r\nLine2")).toBe("Line1\r\n\r\nLine2");
  });

  it("should handle very long strings with newlines", () => {
    const long = "a".repeat(100) + "\n\n" + "b".repeat(100);
    const expected = "a".repeat(100) + "\n" + "b".repeat(100);
    expect(messageUtils.removeRedundantNewlines(long)).toBe(expected);
  });

  it("should handle tabs with newlines", () => {
    expect(messageUtils.removeRedundantNewlines("\t\n\n\t")).toBe("\t\n\t");
  });

  it("should handle special unicode with newlines", () => {
    expect(messageUtils.removeRedundantNewlines("🔥\n\n🔥")).toBe("🔥\n🔥");
  });

  it("should handle multiple patterns", () => {
    expect(messageUtils.removeRedundantNewlines("a\n\nb\n\nc\n\nd")).toBe("a\nb\nc\nd");
  });

  it("should handle complex text", () => {
    const complex = "# Title\n\nParagraph\n\n## Subtitle\n\nMore text";
    const expected = "# Title\nParagraph\n## Subtitle\nMore text";
    expect(messageUtils.removeRedundantNewlines(complex)).toBe(expected);
  });

  it("should be idempotent", () => {
    const input = "Test\n\nTest";
    const once = messageUtils.removeRedundantNewlines(input);
    const twice = messageUtils.removeRedundantNewlines(once);
  ...
  it("should handle triple newlines", () => {
    expect(messageUtils.removeRedundantNewlines("Test\n\n\nTest")).toBe("Test\n\nTest");
  });

  it("should handle quadruple newlines", () => {
    expect(messageUtils.removeRedundantNewlines("Test\n\n\n\nTest")).toBe("Test\n\nTest");
  });
  ...
  it("should handle many consecutive newlines", () => {
    ...
    expect(result.includes("End")).toBe(true);
  });
});
