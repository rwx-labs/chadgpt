...
/** The default tick rate for agentic behavior. */
export const DEFAULT_AGENT_TICK_RATE = 30; // Every 30 seconds
...
/** List of keys in the `agent` map of the configuration that are required. */
export const REQUIRED_AGENT_KEYS = [
  "actions",
  "model",
  "prompt",
  "tick",
...
/**
 * Configuration for agentic behaviour.
 */
export class AgentConfig {
  ...
  /**
   * The frequency in seconds at which the agent will look at the chat messages and perform
   * an action.
   */
  tick = DEFAULT_AGENT_TICK_RATE;

  /**
   * List of actions that the agent is allowed to perform.
   */
  actions = [];
  ...
  /** Create an AgentConfig with keys from the given object.
   *
   * @throws {ValidationError} if the object does not have valid keys
   */
  static from(object) {
    AgentConfig.validate(object);

    return Object.assign(new AgentConfig(), object);
  }
    ...
    for (const key of REQUIRED_AGENT_KEYS) {
      if (!(key in object)) {
        throw new ValidationError(
          `Missing configuration value for agent.${key}!`,
        );
      }
    }
...
export class LlmConfig {
  ...
  /**
   * The maximum number of tokens used in a reasoning context.
   */
...
export class Config {
   ...
   * @see Config.load
   */
  constructor(ircConfig, llmConfig, agentConfig) {
    this.irc = ircConfig;
    this.llm = llmConfig;
  ...
  static async load(path) {
    ...
    for (const key of ["irc", "llm", "agent"]) {
    ...
    const agentConfig = AgentConfig.from(yaml["agent"]);
    const config = new Config(ircConfig, llmConfig, agentConfig);
